...
2. `uv run python scripts/check_final_acceptance_acceptance.py`
3. `cd frontend && npm run validate:routes && npm run build`
4. `docker compose -f deploy/docker/docker-compose.yml config --services`
5. `sh deploy/backup/backup_ops_deployment.sh --smoke`
6. `sh deploy/backup/restore_ops_deployment.sh --smoke`
