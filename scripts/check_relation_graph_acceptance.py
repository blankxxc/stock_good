...
def run_acceptance() -> dict[str, Any]:
    ...
    report = run_relation_graph_relation_graph_pipeline(write_outputs=True)
    ...
    daily = _read_parquet_dir(ROOT / "data" / "gold" / "factor_daily_panel_relation_graph_relation")
    feature = _read_parquet_dir(ROOT / "data" / "gold" / "model_feature_matrix_wide_relation_graph")
    ablation = _read_json(relation_graph_DIR / "relation_factor_ablation_report.json")
    graph_summary = _read_json(relation_graph_DIR / "graph_summary.json")
    ...
    check("relation_graph_feature_matrix_written", not feature.empty and required_factor_cols.issubset(feature.columns) and feature["feature_set_version"].astype(str).str.contains("relation_graph_relation_graph").all())
    ...
    relation_graph_DIR.mkdir(parents=True, exist_ok=True)
    (relation_graph_DIR / "acceptance_report.json").write_text(json.dumps(result, ensure_ascii=False, indent=2, default=_json_default), encoding="utf-8")
    return result
