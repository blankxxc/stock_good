...
def run_acceptance() -> dict[str, Any]:
    ...
    from models.advanced_models_advanced_models import run_advanced_models_advanced_model_pipeline

    report = run_advanced_models_advanced_model_pipeline(write_outputs=True)
    comparison = _read_json(advanced_models_DIR / "model_comparison_report.json")
    ...
    check("advanced_models_report_ok", report.get("status") == "ok")
    ...
    check("experiment_artifacts_written", all((ROOT / "reports" / "advanced_models" / "experiment_recorder" / (item.get("run_id") or item.get("blocked_run_id", "")) / name).is_file() for item in report.get("models", {}).values() for name in ["metrics.json", "artifact_manifest.json", "model_card.json", "feature_dependency.json"]))
    check("backend_models_api_ready", api_models.status_code == 200 and api_models.json().get("status") == "advanced_models_advanced_models_ready")
    check("backend_experiments_api_links_advanced_models", api_experiments.status_code == 200 and api_experiments.json().get("advanced_models", {}).get("status") == "advanced_models_advanced_models_ready")
    check("health_module_ready", health.status_code == 200 and health.json().get("modules", {}).get("advanced_models") == "advanced_models_research_candidate_adapters_ready")
    page = (ROOT / "frontend" / "src" / "app" / "models" / "page.tsx").read_text(encoding="utf-8")
    check("frontend_models_page_advanced_models_ready", "advanced_models" in page and "/api/models" in page and "research candidate" in page and all(name in page for name in REQUIRED_MODELS))
    ...
    advanced_models_DIR.mkdir(parents=True, exist_ok=True)
    (advanced_models_DIR / "acceptance_report.json").write_text(json.dumps(result, ensure_ascii=False, indent=2, default=_json_default), encoding="utf-8")
    return result
