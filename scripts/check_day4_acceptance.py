...
def run_acceptance() -> dict[str, Any]:
    ...
    expected_day4_paths = [
        ...
        ROOT / "lakehouse" / "duckdb" / "day4_factor_queries.sql",
    ]
    ...
    check("single_factor_reports_written", factor_report.get("single_factor_report_count", 0) >= 10 and (ROOT / "reports" / "day4" / "day4_factor_report.html").is_file())
    ...
    check("day4_expected_paths_exist", all(path.is_file() for path in expected_day4_paths))

    api_factors = client.get("/api/factors")
    api_features = client.get("/api/features")
    api_spark = client.get("/api/spark-jobs")
    check("backend_factors_api_ready", api_factors.status_code == 200 and api_factors.json().get("status") == "day4_factor_store_ready")
    check("backend_features_api_ready", api_features.status_code == 200 and api_features.json().get("status") == "day4_feature_registry_ready")
    check("backend_spark_api_ready", api_spark.status_code == 200 and api_spark.json().get("status") == "day4_spark_factor_materialization_ready")
    page_text = factor_page.read_text(encoding="utf-8")
    check("frontend_factor_page_day4_ready", "Day 4" in page_text and "74 个离线因子" in page_text and "/api/factors" in page_text)
    ...
    report_path = ROOT / "reports" / "day4" / "acceptance_report.json"
