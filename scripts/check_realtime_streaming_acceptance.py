...
realtime_streaming_DIR = ROOT / "reports" / "realtime_streaming"
EXPECTED_TOPICS = {
    "raw.market.tick",
    "raw.market.trade",
...
def run_acceptance() -> dict[str, Any]:
    ...
    topic_health = _read_json(realtime_streaming_DIR / "topic_health.json")
    flink_status = _read_json(realtime_streaming_DIR / "flink_job_status.json")
    snapshot = _read_json(realtime_streaming_DIR / "online_feature_snapshot.json")
    diff = _read_json(realtime_streaming_DIR / "realtime_vs_offline_diff_report.json")
    ...
    check("backend_realtime_api_ready", api_realtime.status_code == 200 and api_realtime.json().get("status") == "realtime_streaming_realtime_pipeline_ready")
    check("backend_flink_jobs_api_ready", api_flink.status_code == 200 and api_flink.json().get("status") == "realtime_streaming_flink_jobs_ready")
    ...
    check("frontend_realtime_page_realtime_streaming_ready", "realtime_streaming" in realtime_page and "/api/realtime" in realtime_page and "PoC / not formal signal" in realtime_page)
    check("frontend_flink_page_realtime_streaming_ready", "realtime_streaming" in flink_page and "/api/flink-jobs" in flink_page)
    ...
    (realtime_streaming_DIR / "acceptance_report.json").write_text(json.dumps(result, ensure_ascii=False, indent=2, default=_json_default), encoding="utf-8")
    return result
