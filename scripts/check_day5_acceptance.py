...
def run_acceptance() -> dict[str, Any]:
    ...
    report = run_day5_research_loop(write_outputs=True)
    ...
    predictions = _read_parquet_file(DAY5_DIR / "predictions.parquet")
    holdings = _read_parquet_file(DAY5_DIR / "holdings.parquet")
    risk_report = _read_parquet_file(DAY5_DIR / "risk_report.parquet")
    recorder_dir = DAY5_DIR / "experiment_recorder" / str(report.get("run_id"))
    ...
    check("day5_research_loop_status_ok", report.get("status") == "ok")
    ...
    check("research_artifacts_written", all((DAY5_DIR / name).is_file() for name in ["predictions.parquet", "holdings.parquet", "equity_curve.csv", "risk_report.parquet", "backtest_report.html"]))
    ...
    check("backend_dashboard_scores_backtests_ready", api_dashboard.status_code == 200 and api_dashboard.json().get("status") == "day5_research_loop_ready" and api_scores.status_code == 200 and api_scores.json().get("status") == "day5_scores_ready" and api_backtests.status_code == 200 and api_backtests.json().get("status") == "day5_backtest_ready")
    check("backend_experiments_ready", api_experiments.status_code == 200 and api_experiments.json().get("status") == "day5_experiment_recorder_ready")
    ...
    check("frontend_dashboard_scores_backtests_day5_ready", "Day 5" in dashboard_page and "/api/dashboard" in dashboard_page and "Day 5" in scores_page and "/api/scores" in scores_page and "Day 5" in backtests_page and "/api/backtests" in backtests_page)
    ...
    report_path = DAY5_DIR / "acceptance_report.json"
