...
def run_acceptance() -> dict[str, Any]:
    ...
    from rag.day10_evidence_system import answer_with_evidence, retrieve_claims, run_day10_rag_pipeline

    report = run_day10_rag_pipeline(write_outputs=True)
    ...
    check("day10_report_ok", report.get("status") == "ok")
    ...
    check("backend_rag_api_ready", api_rag.status_code == 200 and api_rag.json().get("status") == "day10_rag_evidence_ready")
    check("health_module_ready", health.status_code == 200 and health.json().get("modules", {}).get("rag") == "day10_claim_evidence_rag_ready")
    check("frontend_rag_page_day10_ready", "Day 10" in page and "/api/rag" in page and "claim_id" in page and "as_of" in page and "无引用拒答" in page)
    ...
    (DAY10_DIR / "acceptance_report.json").write_text(json.dumps(result, ensure_ascii=False, indent=2, default=_json_default), encoding="utf-8")
    return result
