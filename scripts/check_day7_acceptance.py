...
def run_acceptance() -> dict[str, Any]:
    ...
    report = run_day7_event_regime_pipeline(write_outputs=True)
    ...
    feature = _read_parquet_dir(ROOT / "data" / "gold" / "model_feature_matrix_wide_day7")
    ablation = _read_json(DAY7_DIR / "event_regime_ablation_report.json")
    ...
    check("backend_factors_api_has_day7_payload", api_factors.status_code == 200 and api_factors.json().get("event_regime", {}).get("status") == "day7_event_regime_ready")
    factors_page = (ROOT / "frontend" / "src" / "app" / "factors" / "page.tsx").read_text(encoding="utf-8")
    check("frontend_factors_page_day7_ready", "Day 7" in factors_page and "事件因子" in factors_page and "market regime" in factors_page and "publish_time / available_time" in factors_page)
    ...
    check("artifacts_manifest_present", bool(report.get("artifacts")) and (DAY7_DIR / "day7_event_regime_report.json").is_file())
    ...
    (DAY7_DIR / "acceptance_report.json").write_text(json.dumps(result, ensure_ascii=False, indent=2, default=_json_default), encoding="utf-8")
    return result
