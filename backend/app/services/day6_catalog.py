...
def _ensure_day6_report() -> dict[str, Any]:
    root = project_root()
    report_path = root / "reports" / "day6" / "day6_realtime_report.json"
    ...
    try:
        ...
        return {"status": "day6_realtime_pending", "error": str(exc)}


def realtime_payload(research_boundary: str) -> dict[str, Any]:
    root = project_root()
    report = _ensure_day6_report()
    topic_health = _read_json(root / "reports" / "day6" / "topic_health.json")
    snapshot = _read_json(root / "reports" / "day6" / "online_feature_snapshot.json")
    factors = _read_parquet(root / "reports" / "day6" / "realtime_factor_latest.parquet")
    if report.get("status") != "ok" or factors.empty:
        return {
            "module": "realtime",
            "status": "day6_realtime_pending",
    ...
    return {
        "module": "realtime",
        "status": "day6_realtime_pipeline_ready",
...
def flink_jobs_payload(research_boundary: str) -> dict[str, Any]:
    root = project_root()
    report = _ensure_day6_report()
    job_status = _read_json(root / "reports" / "day6" / "flink_job_status.json")
    jobs = job_status.get("jobs", [])
    if report.get("status") != "ok" or not jobs:
        return {
            "module": "flink-jobs",
            "status": "day6_flink_jobs_pending",
            "maturity": "L1-route-stub",
            "research_boundary": research_boundary,
            "error": report.get("error"),
        }
