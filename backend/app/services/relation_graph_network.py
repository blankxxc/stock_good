...
def _recent_daily(connection: duckdb.DuckDBPyConnection, daily_path: Path) -> pd.DataFrame:
    source = _duckdb_path(daily_path)
    return connection.execute(
        f"""
        SELECT
            CAST(trade_date AS VARCHAR) AS trade_date,
            CAST(symbol AS VARCHAR) AS symbol,
            COALESCE(NULLIF(CAST(stock_name AS VARCHAR), ''), CAST(name AS VARCHAR), CAST(symbol AS VARCHAR)) AS stock_name,
...
def _community_assignments(symbols: list[str], edges: Iterable[dict[str, Any]]) -> dict[str, dict[str, int]]:
    ...
    for target_count in AVAILABLE_COMMUNITY_COUNTS:
        communities = list(
        ...
        while len(communities) < target_count:
            largest_index = max(range(len(communities)), key=lambda index: (len(communities[index]), min(communities[index])))
...
def relation_network_payload(root_value: str) -> dict[str, Any]:
    ...
    edge_path = root / "data" / "gold" / "stock_relation_edge" / "part-000.parquet"
    ...
    try:
        daily = _recent_daily(connection, daily_path)
        ...
        _existing_edges(connection, edge_path, universe, edge_store)
        ...
        return {
            ...
            "nodes": nodes,
            "edges": edges,
            "available_node_counts": [count for count in AVAILABLE_NODE_COUNTS if count <= len(nodes)],
            "available_community_counts": [count for count in AVAILABLE_COMMUNITY_COUNTS if count <= len(nodes)],
            "default_node_count": min(DEFAULT_NODE_COUNT, len(nodes)),
            "default_community_count": DEFAULT_COMMUNITY_COUNT if len(nodes) >= DEFAULT_COMMUNITY_COUNT else len(nodes),
        }
    finally:
        connection.close()
