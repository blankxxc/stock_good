...
def event_regime_payload(research_boundary: str) -> dict[str, Any]:
    root = project_root()
    report = _read_json(root / "reports" / "event_regime" / "event_regime_event_regime_report.json")
    ablation = _read_json(root / "reports" / "event_regime" / "event_regime_ablation_report.json")
    if report.get("status") == "ok":
        return {
            "module": "event-regime",
            "status": "event_regime_event_regime_ready",
    ...
    return {
        "module": "event-regime",
        "status": "event_regime_event_regime_pending",
