...
lakehouse 已完成，并已通过本地验收、前端构建、PySpark local 验证、ClickHouse ADS 装载、Docker Compose 运行态与关键 HTTP/API 烟测。
...
- 本地批量数据接入：`scripts/run_lakehouse_pipeline.py`
  - 生成 synthetic lakehouse 样例数据。
  - 落地 Bronze/ODS、Silver/DWD、Gold/DWS、ADS Parquet。
- ODS raw tables：11 张。
- DWD tables：5 张。
- Gold/DWS + ADS tables：14 张。
- Snapshot manifest：`data/snapshots/dataset_snapshot_manifest_lakehouse.json`
  - 当前 30 个 snapshot。
...
- DuckDB 研究查询：`lakehouse/duckdb/lakehouse_research_queries.sql`。
- Spark local jobs：
...
- ClickHouse ADS：
  - DDL：`deploy/clickhouse/lakehouse_ads_tables.sql`
  - Loader：`scripts/load_lakehouse_clickhouse.py`
  - 已装载：`ads_dashboard_summary=1`，`ads_score_latest=4`，`ads_backtest_summary=1`。
- Backend API：
  - `/api/licenses` -> `lakehouse_license_registry_ready`
  - `/api/lakehouse` -> `lakehouse_lakehouse_ready`
  - `/api/data-quality` -> `lakehouse_ads_quality_summary_ready`
- Frontend pages：
  - `/dashboard`
  - `/lakehouse`
  - `/settings/licenses`
...
```bash
.venv/Scripts/python.exe scripts/check_lakehouse_acceptance.py
```
...
- `scripts/run_lakehouse_pipeline.py`：通过。
...
- `pytest tests/test_foundation_scaffold.py tests/test_lakehouse_batch_lakehouse.py tests/test_iceberg_table_format.py -q`：15 passed。
- `npm run build`：Next.js production build 成功，23 个静态页面生成。
- `docker compose config --quiet`：通过。
- `scripts/load_lakehouse_clickhouse.py`：通过。
- ClickHouse `SELECT count() FROM ads_dashboard_summary`：返回 1。

完整报告：`reports/lakehouse/acceptance_report.json`。
...
- Frontend `/`：HTTP 200。
- Frontend `/settings/licenses`：HTTP 200，页面包含 `lakehouse`。
- Backend `/health`：HTTP 200。
- Backend `/api/licenses`：HTTP 200，`lakehouse_license_registry_ready`。
- Backend `/api/lakehouse`：HTTP 200，`lakehouse_lakehouse_ready`。
- Spark UI：HTTP 200。
- Flink UI：HTTP 200。
...
- lakehouse licenses page: http://127.0.0.1:3000/settings/licenses
- Backend health: http://127.0.0.1:8000/health
- lakehouse licenses API: http://127.0.0.1:8000/api/licenses
- lakehouse lakehouse API: http://127.0.0.1:8000/api/lakehouse
...
- lakehouse 使用 synthetic/local sample 数据，不伪造任何外部供应商授权。
- 受限/未授权源只进入 registry 和 schema/adapter contract，不进入可展示数据。
- Iceberg connector 已通过 `spark.jars.packages` 解析并缓存到本机 Ivy：`org.apache.iceberg:iceberg-spark-runtime-3.5_2.12:1.11.0`；单独验收报告为 `reports/lakehouse/iceberg_table_format_acceptance.json`。
