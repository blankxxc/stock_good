"""Day 3 data-trust layer for the intelligent stock research platform.
...
DATA_VERSION = "day3_v001"
SOURCE_VERSION = "synthetic_mini_market_v001"
SCHEMA_VERSION = "v0.3.0"
CREATED_BY = "hermes-day3-data-trust"
...
<head>
<meta charset="utf-8" />
<title>Day 3 Data Quality Report</title>
<style>
...
<body>
<span class="badge">Day 3 · data_quality · quarantine · leakage_check_status</span>
<h1>Day 3 数据质量报告</h1>
<div class="card">
...
<head>
<meta charset="utf-8" />
<title>Day 3 Lineage Report</title>
<style>
body {{ font-family: Inter, Arial, sans-serif; margin: 32px; background: #0f172a; color: #e5e7eb; }}
.card {{ background: #111827; border: 1px solid #334155; border-radius: 16px; padding: 20px; margin: 16px 0; }}
table {{ border-collapse: collapse; width: 100%; font-size: 13px; }}
...
<body>
<span class="badge">Day 3 · source_table → transform_job → target_table → report/model/backtest</span>
<h1>Day 3 血缘报告</h1>
<div class="card">
  <p>status: <strong>{_html_escape(report['status'])}</strong></p>
    ...
    for day_idx, trade_date in enumerate(dates):
        ...
        for sym_idx, symbol in enumerate(symbols):
            ...
            rows.append({
                ...
                "trace_id": f"day3-{day_idx:03d}-{sym_idx:03d}",
            })
...
def _write_synthetic_artifacts(clean: pd.DataFrame, traps: pd.DataFrame, scenario_flags: dict[str, Any]) -> dict[str, Any]:
    ...
    market_path = _write_single_parquet(clean, sample_root / "day3_market_daily.parquet")
    traps_path = _write_single_parquet(traps, sample_root / "day3_intentional_traps.parquet")
    ...
    _write_json(ROOT / "reports" / "day3" / "synthetic_mini_market_report.json", report)
    return report
...
def _quarantine_records(traps: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in traps.iterrows():
        ...
        records.append({
            ...
            "resolution_note": "Intentional Day3 synthetic anomaly quarantined before training/backtest/report use.",
...
def _write_quarantine(records: pd.DataFrame) -> dict[str, Any]:
    quarantine_root = ROOT / "data" / "quarantine" / "day3_synthetic_market"
    ...
    summary_path = ROOT / "reports" / "day3" / "quarantine_summary.json"
    summary = {
        "status": "written",
...
def _build_quality_checks(clean: pd.DataFrame, quarantine_summary: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        _check_result("schema_match", REQUIRED_MARKET_COLUMNS.issubset(clean.columns), sorted(REQUIRED_MARKET_COLUMNS - set(clean.columns)), "missing=[]", "Clean synthetic market exposes Day3 point-in-time schema."),
...
def _write_quality_report(clean: pd.DataFrame, quarantine_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    _write_json(ROOT / "reports" / "day3" / "data_quality_report.json", report)
    _write_quality_html(report, ROOT / "reports" / "data_quality_report.html")
    _write_quality_html(report, ROOT / "reports" / "day3" / "data_quality_report.html")
    return report
...
def _write_leakage_report(clean: pd.DataFrame, traps: pd.DataFrame) -> dict[str, Any]:
    ...
    _write_json(ROOT / "reports" / "day3" / "leakage_report.json", report)
    return report
...
def _write_lineage_report(quality_report: dict[str, Any], leakage_report: dict[str, Any]) -> dict[str, Any]:
    manifest_path = ROOT / "data" / "snapshots" / "dataset_snapshot_manifest_day2.json"
    ...
    for row in manifest:
        ...
        transform_job = "job:day2_materialize_ods" if layer == "ODS" else f"job:day2_{layer.lower()}_transform"
        ...
        if layer == "ODS":
            ...
            edges.append(_edge(source_node, "source_table", transform_job, "transform_job", "ingested_by", "day2_batch_ingest"))
            edges.append(_edge(transform_job, "transform_job", table_node, "target_table", "writes_table", "day2_batch_ingest"))
        else:
            for upstream in row.get("upstream_snapshot_ids", []):
                ...
                edges.append(_edge(upstream_node, "snapshot", transform_job, "transform_job", "read_by", "day2_transform"))
            edges.append(_edge(transform_job, "transform_job", table_node, "target_table", "writes_table", "day2_transform"))
    ...
    spark_targets = {
        "reports/day2/spark_bronze_to_silver_market_daily_report.json": ["dwd_stock_daily_bar"],
        "reports/day2/spark_bronze_to_silver_reference_report.json": ["dwd_financial_statement", "dwd_news_event", "dwd_announcement_event"],
        "reports/day2/spark_silver_to_gold_base_panels_report.json": ["factor_daily_panel", "label_cross_sectional_return", "model_training_sample"],
    }
    ...
    quality_job = "job:day3_data_quality_check"
    leakage_job = "job:day3_leakage_checker"
    lineage_job = "job:day3_lineage_builder"
    ...
    _node(nodes, "reports/day3/leakage_report.json", "report", status=leakage_report.get("status"))
    ...
    for dataset in ["dwd_stock_daily_bar", "factor_daily_panel", "model_training_sample", "ads_data_quality_summary"]:
        ...
        if snap:
            edges.append(_edge(f"snapshot:{snap}", "snapshot", quality_job, "transform_job", "checked_by", "day3_quality"))
    edges.append(_edge(quality_job, "transform_job", "reports/data_quality_report.json", "report", "writes_report", "day3_quality"))
    edges.append(_edge(quality_job, "transform_job", leakage_job, "transform_job", "triggers_leakage_check", "day3_quality"))
    edges.append(_edge(leakage_job, "transform_job", "reports/day3/leakage_report.json", "report", "writes_report", "day3_leakage"))
    edges.append(_edge(lineage_job, "transform_job", "reports/lineage_report.json", "report", "writes_report", "day3_lineage"))
    edges.append(_edge("reports/data_quality_report.json", "report", "reports/lineage_report.json", "report", "referenced_by", "day3_lineage"))

    report = {
        "status": "passed",
        "day": 3,
        "data_version": DATA_VERSION,
        "source_manifest_path": "data/snapshots/dataset_snapshot_manifest_day2.json",
    ...
    _write_json(ROOT / "reports" / "day3" / "lineage_report.json", report)
    _write_lineage_html(report, ROOT / "reports" / "lineage_report.html")
    _write_lineage_html(report, ROOT / "reports" / "day3" / "lineage_report.html")
    return report


def run_day3_data_trust() -> dict[str, Any]:
    """Generate all Day 3 data quality, lineage, quarantine and leakage artifacts."""
    if str(ROOT) not in sys.path:
        sys.path.insert(0, str(ROOT))
    # Day 3 depends on Day 2 lakehouse artifacts. Build them if a fresh clone has none.
    day2_report_path = ROOT / "reports" / "day2" / "day2_pipeline_report.json"
    if not day2_report_path.exists():
    ...
    report = {
        ...
        "artifacts": {
            ...
            "quarantine": "data/quarantine/day3_synthetic_market",
        },
        "content_hash": _content_hash(clean)[:24],
        "research_boundary": RESEARCH_BOUNDARY,
        "generated_at": _utc_now(),
    }
    _write_json(ROOT / "reports" / "day3" / "day3_data_trust_report.json", report)
    markdown = f"""# Day 3 数据可信度完成报告
...
- reports/day3/leakage_report.json
- reports/day3/synthetic_mini_market_report.json
- data/quarantine/day3_synthetic_market

研究边界：{RESEARCH_BOUNDARY}。
"""
    (ROOT / "reports" / "day3").mkdir(parents=True, exist_ok=True)
    (ROOT / "reports" / "day3" / "day3_completion_summary.md").write_text(markdown, encoding="utf-8")
    return report
