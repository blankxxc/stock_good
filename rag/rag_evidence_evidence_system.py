...
INDEX_VERSION = "rag_index_rag_evidence_v001"
...
def _base_documents() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "source_url": "file://reports/research_loop/backtest_report.html",
        ...
        {
            "doc_id": "experiment_relation_graph_001",
            "doc_type": "experiment_card",
            "title": "relation_graph relation factor ablation experiment",
            "source_url": "file://reports/relation_graph/relation_factor_ablation_report.json",
            ...
            "text": "relation_graph relation_spillover ablation 未观察到稳定正增益，结论只能标记为 research candidate，不得进入 approved。",
        },
        {
            "doc_id": "backtest_research_loop_001",
            "doc_type": "backtest_report",
            "title": "research_loop tradable backtest risk report",
            "source_url": "file://reports/research_loop/backtest_report.html",
        ...
        },
        {
            ...
            "source_url": "file://reports/data_trust/leakage_report.json",
        ...
        },
        {
            ...
            "source_url": "file://reports/relation_graph/relation_graph_relation_graph_report.json",
...
def _claim_for_document(doc: dict[str, Any], ordinal: int) -> dict[str, Any]:
    ...
    row = {
        ...
        "related_experiment_ids": ["relation_graph_relation_ablation"] if doc["doc_type"] == "experiment_card" else [],
        "related_run_ids": ["relation_graph_relation_ablation_v001"] if doc["doc_type"] == "experiment_card" else [],
...
def build_rag_evidence_tables() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def _write_schema() -> None:
    schema = {
        ...
        "layer": "RAG",
        "schema_version": SCHEMA_VERSION,
        "description": "rag_evidence claim-level RAG evidence schema with point-in-time retrieval, citation spans, license gates, and index metadata.",
...
def _write_citation_html(cards: list[dict[str, Any]], path: Path) -> None:
    ...
    html = "<!doctype html><html lang='zh-CN'><meta charset='utf-8'><title>rag_evidence RAG Citation Cards</title><body><h1>rag_evidence RAG Citation Cards</h1>" + "\n".join(rows) + "</body></html>"
    path.write_text(html, encoding="utf-8")


def run_rag_evidence_rag_pipeline(*, write_outputs: bool = True) -> dict[str, Any]:
    docs, chunks, claims = build_rag_evidence_tables()
    if write_outputs:
        _write_schema()
        _write_eval_sets()
    ...
    report_dir = PROJECT_ROOT / "reports" / "rag_evidence"
    ...
    report = {
        ...
        "artifacts": {
            ...
            "rag_eval_report": "reports/rag_evidence/rag_eval_report.json",
            "rag_answer_cards": "reports/rag_evidence/rag_answer_cards.json",
            "rag_citation_cards": "reports/rag_evidence/rag_citation_cards.html",
            "rag_evidence_report": "reports/rag_evidence/rag_evidence_report.json",
...
if __name__ == "__main__":
    print(json.dumps(run_rag_evidence_rag_pipeline(write_outputs=True), ensure_ascii=False, indent=2))
