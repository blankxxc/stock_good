...
def _base_event(*, topic: str, symbol: str, event_time: datetime, source: str, payload: dict[str, Any], exchange: str = "SSE") -> dict[str, Any]:
    ...
    trace_id = f"day6-{topic}-{symbol}-{event_time.strftime('%Y%m%d%H%M%S')}"
    return {
...
def build_day6_replay_events() -> list[TopicEvent]:
...
def produce_day6_replay(*, reset: bool = True) -> dict[str, Any]:
    return write_events(build_day6_replay_events(), reset=reset)
