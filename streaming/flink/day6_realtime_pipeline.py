...
from streaming.producers.day6_replay_producer import FEED_MODE, ROOT, TOPIC_LOG_DIR, expected_topics, produce_day6_replay

REPORT_DIR = ROOT / "reports" / "day6"
GOLD_INTRADAY_DIR = ROOT / "data" / "gold" / "factor_intraday_panel"
RESEARCH_BOUNDARY = "research_signals_only_not_investment_advice"
FACTOR_VERSION = "rt_factor_day6_v001"
MATURITY = "PoC"
...
def _write_topic(topic: str, rows: list[dict[str, Any]]) -> None:
    ...
    with path.open("w", encoding="utf-8") as fh:
        for row in rows:
            ...
            event.setdefault("source", f"day6_local_{topic}")
            event.setdefault("symbol", symbol)
            event.setdefault("exchange", "CN")
            event.setdefault("trade_date", str(event_time)[:10])
            event.setdefault("trace_id", f"day6-{topic}-{symbol}-{str(event_time).replace(':', '').replace('-', '')}")
            event.setdefault("schema_version", "day6.v1")
...
def build_diff_report(factors: pd.DataFrame) -> dict[str, Any]:
    ...
    html = "<html><body><h1>Day 6 realtime vs offline recompute diff</h1>" + f"<p>rows={report['rows_compared']} max_abs_diff={report['max_abs_diff']}</p><p>feed_mode={FEED_MODE}</p></body></html>"
    (REPORT_DIR / "realtime_vs_offline_diff_report.html").write_text(html, encoding="utf-8")
    return report


def run_day6_realtime_pipeline(*, write_outputs: bool = True) -> dict[str, Any]:
    producer_report = produce_day6_replay(reset=True)
    ...
    if factors.empty:
        raise RuntimeError("Day6 pipeline produced no realtime factors")
    ...
    flink_status = {"status": "day6_flink_jobs_ready", "jobs": flink_jobs, "job_count": len(flink_jobs), "savepoint_status": "not_required_for_local_poc", "maturity": MATURITY}
    ...
    if write_outputs:
        ...
        (REPORT_DIR / "day6_realtime_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2, default=_json_default), encoding="utf-8")
    return report


if __name__ == "__main__":
    print(json.dumps(run_day6_realtime_pipeline(write_outputs=True), ensure_ascii=False, indent=2, default=_json_default))
