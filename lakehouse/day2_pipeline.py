...
from data.adapters.day2_sources import registry_as_dicts, status_counts, write_registry

ROOT = Path(__file__).resolve().parents[1]
DATA_VERSION = "day2_v001"
SOURCE_VERSION = "synthetic_v001"
SCHEMA_VERSION = "v0.2.0"
CREATED_BY = "hermes-day2-local-pipeline"
...
def build_source_tables() -> dict[str, pd.DataFrame]:
    ...
    industry = pd.DataFrame([
        {"symbol": s, "industry_level": "L1", "industry_name": ind, "as_of_date": "2026-01-01", "source": "synthetic_day2"}
...
def _materialize_ods(source_tables: dict[str, pd.DataFrame]) -> tuple[dict[str, pd.DataFrame], list[dict[str, Any]], dict[str, str]]:
    ...
    for ods_name, source_name in ODS_SOURCE_MAP.items():
        ...
        base = ROOT / "data" / "bronze" / "synthetic_day2" / ods_name
...
def _build_gold_ads(dwd_tables: dict[str, pd.DataFrame], dwd_snapshot_ids: dict[str, str], snapshots_so_far: list[dict[str, Any]]) -> tuple[dict[str, pd.DataFrame], list[dict[str, Any]]]:
    ...
    backtest = pd.DataFrame([
        {"run_id": "bt_day2_demo_top2", "start_date": daily["trade_date"].min(), "end_date": latest_date, "topk": 2, "long_short_return": 0.012, "turnover": 0.33, "max_drawdown": -0.004, "sharpe": 1.2, "data_version": DATA_VERSION, "factor_version": "v001", "model_version": "baseline_linear_v001"}
    ])
...
def run_pipeline() -> dict[str, Any]:
    ...
    manifest_path = snapshots_dir / "dataset_snapshot_manifest_day2.json"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    pd.DataFrame(manifest).to_parquet(snapshots_dir / "dataset_snapshot_manifest_day2.parquet", index=False)
    ...
    report_path = ROOT / "reports" / "day2" / "day2_pipeline_report.json"
