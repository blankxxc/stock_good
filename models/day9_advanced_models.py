...
MODEL_SPECS: dict[str, ModelSpec] = {
    ...
    "StockMixer": ModelSpec(
        ...
        input_dependency="indicator/temporal/stock mixing tensors from Day8 feature matrix",
        base_features=(
            "return_5d",
            "return_10d",
            "return_20d",
    ...
    "HIST": ModelSpec(
        ...
        input_dependency="concept_matrix + industry/concept/relation spillover features from Day8 graph adapter",
        base_features=(
            "industry_spillover",
            "concept_spillover",
            "centrality_score",
    ...
    "TRSR": ModelSpec(
        ...
        input_dependency="relation_matrix + lead_lag / neighbor propagation features from Day8 graph adapter",
        base_features=(
            "lead_lag_signal",
            "neighbor_return_5m",
            "neighbor_return_1d",
...
def prepare_day9_dataset() -> tuple[pd.DataFrame, list[str], dict[str, Any]]:
    if not (ROOT / "reports" / "day8" / "day8_relation_graph_report.json").exists():
        from graph.day8_relation_graph import run_day8_relation_graph_pipeline

        run_day8_relation_graph_pipeline(write_outputs=True)
    features = _read_parquet_dir(ROOT / "data" / "gold" / "model_feature_matrix_wide_day8")
    ...
    if len(sample["trade_date"].unique()) < 20:
        raise AssertionError("Day9 needs at least 20 trading dates for small-sample train/valid/test split")
    metadata = {
        "rows": int(len(sample)),
...
class Day9AdvancedModelAdapter:
    ...
    def register_model_artifact(self, run_id: str, metrics: dict[str, Any], predictions_path: str) -> dict[str, Any]:
        ...
        _write_json(run_dir / "artifact_manifest.json", {"run_id": run_id, "recorder_type": "file_based_mlflow_qlib_compatible_day9", "artifacts": artifacts})
    ...
    def explain_feature_dependency(self, run_id: str) -> dict[str, Any]:
        dependency = {
            ...
            "day8_adapter_dependency": "data/gold/graph_model_adapters/hist_trsr" if self.spec.relation_required else None,
...
def _baseline_lightgbm_summary() -> dict[str, Any]:
    report_path = ROOT / "reports" / "day5" / "day5_research_loop_report.json"
    if not report_path.exists():
        return {"status": "missing", "BlockedReason": "Day5 LightGBM report is missing"}
    ...
    metrics.update(
        {
            "RuntimeSeconds": None,
            "ParameterCount": None,
            "TrainingCostTier": "baseline-day5",
...
def build_adapter(seed: int = 42) -> Day9AdvancedModelAdapter:
    """Return the Day 9 local small-sample {spec.name} adapter."""
...
'''
    readme = f"""# {spec.name} Day9 adapter
...
"""
    ...
    (model_dir / "blocked_reason.md").write_text(
        "Official production repo integration was intentionally not cloned for Day9 L1. "
        "The local adapter ran on the unified Day8 feature matrix; promotion requires dependency review, "
...
def _write_model_card(spec: ModelSpec, run_id: str, metrics: dict[str, Any], summary: dict[str, Any]) -> None:
    card = {
        ...
        "risk_notes": [
            ...
            "must pass Day12 simulation/risk and Day14 review before any promotion",
        ],
        "research_boundary": RESEARCH_BOUNDARY,
        "generated_at": _now(),
    }
...
def train_one_model(spec: ModelSpec, sample: pd.DataFrame, splits: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    final_adapter: Day9AdvancedModelAdapter | None = None
    ...
    run_id = f"day9_{spec.slug}_small_sample_v001"
    for seed in SEEDS:
        adapter = Day9AdvancedModelAdapter(spec, seed=seed)
...
def _write_comparison_report(model_summaries: dict[str, dict[str, Any]], prediction_frames: list[pd.DataFrame]) -> dict[str, Any]:
    ...
    report = {
        ...
        "artifacts": {
            ...
            "model_comparison_report": "reports/day9/model_comparison_report.json",
        },
    }
    _write_json(DAY9_DIR / "model_comparison_report.json", report)
    return report


def run_day9_advanced_model_pipeline(write_outputs: bool = True) -> dict[str, Any]:
    sample, _, dataset_meta = prepare_day9_dataset()
    ...
    report = {
        ...
        "run_id": "day9_advanced_model_integration_v001",
        ...
        "artifacts": {
            ...
            "model_comparison_report": "reports/day9/model_comparison_report.json",
            "model_comparison_csv": "data/gold/advanced_model_comparison/model_comparison.csv",
            "experiment_recorder": "reports/day9/experiment_recorder",
        },
    }
    if write_outputs:
        _write_parquet_dir(predictions, PREDICTION_DIR)
        _write_json(DAY9_DIR / "advanced_model_integration_report.json", report)
    return report
...
if __name__ == "__main__":
    print(json.dumps(run_day9_advanced_model_pipeline(write_outputs=True), ensure_ascii=False, indent=2, default=_json_default))
