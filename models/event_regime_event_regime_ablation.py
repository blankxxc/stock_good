...
DATA_VERSION = "event_regime_synthetic_event_regime_v001"
SCHEMA_VERSION = "v0.7.0"
EVENT_FACTOR_VERSION = "event_factor_event_regime_v001"
MARKET_REGIME_VERSION = "market_regime_event_regime_v001"
FEATURE_SET_VERSION = "feature_set_event_regime_event_regime_v001"
MODEL_VERSION = "lightgbm_event_regime_event_regime_smoke_v001"
RUN_ID = "event_regime_event_regime_ablation_v001"
...
ENHANCED_FEATURE_DIR = ROOT / "data" / "gold" / "model_feature_matrix_wide_event_regime"
FACTOR_DAILY_event_regime_DIR = ROOT / "data" / "gold" / "factor_daily_panel_event_regime_event_regime"
...
def build_event_documents(feature: pd.DataFrame, write_outputs: bool = True) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for idx, date in enumerate(anchor_dates):
        ...
        news_rows.append(
            {
                "document_id": f"news_event_regime_{idx + 1:03d}",
                "document_type": "news",
        ...
        if idx % 2 == 0:
            ...
            ann_rows.append(
                {
                    "document_id": f"ann_event_regime_{ann_idx + 1:03d}",
...
def build_event_extractions(news: pd.DataFrame, ann: pd.DataFrame, trading_dates: list[str], write_outputs: bool = True) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for idx, row in docs.iterrows():
        ...
        rows.append(
            {
                "event_id": f"event_event_regime_{idx + 1:03d}",
...
def run_ablation(enhanced: pd.DataFrame, write_outputs: bool = True) -> tuple[dict[str, Any], str]:
    ...
    report = {
        ...
        "leakage_check_status": "passed",
        "relation_spillover_note": "event_regime keeps relation_spillover as a zero placeholder; relation_graph will replace it with graph propagation factors.",
        "research_boundary": RESEARCH_BOUNDARY,
        "generated_at": _now(),
    }
    if write_outputs:
        _write_json(event_regime_DIR / "event_regime_ablation_report.json", report)
...
def _write_ablation_html(report: dict[str, Any]) -> None:
    ...
    html = f"""<!doctype html>
<html lang=\"zh-CN\"><head><meta charset=\"utf-8\"><title>event_regime Event/Regime Ablation</title>
...
<body><span class=\"badge\">event_regime event/regime smoke ablation</span><h1>事件因子、市场环境因子入模 ablation</h1>
...
<div class=\"card\"><h2>治理边界</h2><pre>{json.dumps({k: v for k, v in report.items() if k != 'configs'}, ensure_ascii=False, indent=2, default=_json_default)}</pre></div>
    ...
    (event_regime_DIR / "event_regime_ablation_report.html").write_text(html, encoding="utf-8")


def write_factor_daily_panel_event_regime(event_long: pd.DataFrame, market: pd.DataFrame, write_outputs: bool = True) -> pd.DataFrame:
    ...
    if write_outputs:
        _write_parquet_dir(combined, FACTOR_DAILY_event_regime_DIR)
    return combined


def run_event_regime_event_regime_pipeline(write_outputs: bool = True) -> dict[str, Any]:
    ...
    combined_long = write_factor_daily_panel_event_regime(event_long, market_regime, write_outputs=write_outputs)
    ...
    report = {
        ...
        "artifacts": {
            ...
            "factor_daily_panel_event_regime_event_regime": str(FACTOR_DAILY_event_regime_DIR.relative_to(ROOT)),
            "model_feature_matrix_wide_event_regime": str(ENHANCED_FEATURE_DIR.relative_to(ROOT)),
            "ablation_report": "reports/event_regime/event_regime_ablation_report.json",
            "ablation_report_html": "reports/event_regime/event_regime_ablation_report.html",
        },
        "research_boundary": RESEARCH_BOUNDARY,
        "generated_at": _now(),
    }
    if write_outputs:
        _write_json(event_regime_DIR / "event_regime_event_regime_report.json", report)
    return report


def main() -> None:
    print(json.dumps(run_event_regime_event_regime_pipeline(write_outputs=True), ensure_ascii=False, indent=2, default=_json_default))
