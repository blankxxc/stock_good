...
MODEL_VERSION = "lightgbm_day5_v001"
BACKTEST_VERSION = "backtest_day5_v001"
RUN_ID = "day5_lightgbm_walk_forward_v001"
EXPERIMENT_ID = "exp_day5_cross_sectional_baseline_v001"
RANDOM_SEED = 42
HORIZONS = [5, 10]
PRIMARY_HORIZON = 5
TOP_K = 5
...
DAY5_REPORT_DIR = ROOT / "reports" / "day5"
RECORDER_DIR = DAY5_REPORT_DIR / "experiment_recorder" / RUN_ID
ACCEPTANCE_REPORT = DAY5_REPORT_DIR / "acceptance_report.json"
...
def build_labels(write_outputs: bool = True) -> tuple[pd.DataFrame, dict[str, Any]]:
        ...
        out = pd.DataFrame(
            {
                ...
                "symbol": df["symbol"],
                "horizon": df["horizon"],
                # Backward-compatible Day2 ADS alias: the actual Day5 multi-horizon
    ...
    if write_outputs:
        ...
        _write_json(DAY5_REPORT_DIR / "label_report.json", report)
...
def _walk_forward_splits(sample: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    if len(dates) < 30:
        raise AssertionError(f"Need at least 30 labelled dates for Day5 smoke walk-forward, got {len(dates)}")
...
def _fit_predict_lightgbm(sample: pd.DataFrame, feature_cols: list[str], splits: list[dict[str, Any]]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for split in splits:
        ...
        if lightgbm_available:
            ...
            # Deterministic linear fallback: enough to keep the research loop executable if LightGBM is absent.
        ...
        rows.append(
            {
                ...
                "risk_model_version": "risk_day4_v001",
                "research_boundary": RESEARCH_BOUNDARY,
            }
        )
...
def _write_report_html(metrics: dict[str, Any], curve: pd.DataFrame, holdings: pd.DataFrame, model_metadata: dict[str, Any]) -> None:
    DAY5_REPORT_DIR.mkdir(parents=True, exist_ok=True)
    top = holdings.sort_values(["trade_date", "rank"]).head(20) if not holdings.empty else pd.DataFrame()
    html = f"""<!doctype html>
<html lang=\"zh-CN\">
<head><meta charset=\"utf-8\"><title>Day5 Cross-sectional Research Loop</title>
...
<body>
<span class=\"badge\">Day 5 L2 research loop draft</span>
...
<div class=\"card\"><h2>TopK 持仓样例</h2>{top[['trade_date','symbol','rank','score','weight','forward_return','capacity_at_1pct_adv']].to_html(index=False) if not top.empty else '<p>无</p>'}</div>
    ...
    (DAY5_REPORT_DIR / "backtest_report.html").write_text(html, encoding="utf-8")
...
def _write_recorder(config: dict[str, Any], metrics: dict[str, Any], model_metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "artifacts": {
            ...
            "predictions": "reports/day5/predictions.parquet",
            "holdings": "reports/day5/holdings.parquet",
            "equity_curve": "reports/day5/equity_curve.csv",
            "risk_report": "reports/day5/risk_report.parquet",
            "backtest_report": "reports/day5/backtest_report.html",
        },
    }
...
def run_day5_research_loop(write_outputs: bool = True) -> dict[str, Any]:
    ...
    factor_report_path = ROOT / "reports" / "day4" / "day4_factor_report.json"
    if not factor_report_path.exists():
        materialize_factor_store(write_outputs=True)
    feature_report_path = ROOT / "reports" / "day4" / "point_in_time_join_report.json"
    ...
    if write_outputs:
        ...
        predictions.to_parquet(DAY5_REPORT_DIR / "predictions.parquet", index=False)
        holdings.to_parquet(DAY5_REPORT_DIR / "holdings.parquet", index=False)
        curve.to_csv(DAY5_REPORT_DIR / "equity_curve.csv", index=False, encoding="utf-8")
        risk_report.to_parquet(DAY5_REPORT_DIR / "risk_report.parquet", index=False)
    ...
    if write_outputs:
        _write_json(DAY5_REPORT_DIR / "day5_research_loop_report.json", acceptance)
    return acceptance


def main() -> None:
    print(json.dumps(run_day5_research_loop(write_outputs=True), ensure_ascii=False, indent=2, default=_json_default))
