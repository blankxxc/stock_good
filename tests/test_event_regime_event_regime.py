...
def _ensure_event_regime() -> dict:
    from models.event_regime_event_regime_ablation import run_event_regime_event_regime_pipeline

    return run_event_regime_event_regime_pipeline(write_outputs=True)
...
def test_event_regime_event_documents_and_entity_mapping_are_ready():
    report = _ensure_event_regime()
...
def test_event_regime_financial_text_event_factor_panels_have_time_semantics():
    report = _ensure_event_regime()
...
def test_event_regime_market_regime_features_and_ablation_are_ready():
    report = _ensure_event_regime()
    ...
    feature = _read_parquet_dir(PROJECT_ROOT / "data" / "gold" / "model_feature_matrix_wide_event_regime")
    ablation = json.loads((PROJECT_ROOT / "reports" / "event_regime" / "event_regime_ablation_report.json").read_text(encoding="utf-8"))
...
def test_event_regime_backend_frontend_and_acceptance_are_ready():
    result = _ensure_event_regime()
