...
class IsolatedAuthTestClient(TestClient):
...
def authenticated_admin_client(app) -> TestClient:
    """Return a cookie-authenticated admin client backed by an isolated temporary database."""
    temporary = tempfile.TemporaryDirectory(prefix="stock-good-test-auth-")
    ...
    client = IsolatedAuthTestClient(app, service, temporary, base_url="http://testserver")
