...
def test_day2_source_registry_tracks_priority_sources_and_license_gates():
    registry_path = PROJECT_ROOT / "configs" / "data" / "source_license_registry.yaml"
    adapter_path = PROJECT_ROOT / "data" / "adapters" / "day2_sources.py"
...
def test_day2_output_layers_and_snapshot_manifest_are_materialized():
    report = _read_json("reports/day2/day2_pipeline_report.json")
    assert report["status"] == "ok"
    assert report["data_version"].startswith("day2_")

    manifest = _read_json("data/snapshots/dataset_snapshot_manifest_day2.json")
...
def test_day2_core_parquet_tables_are_queryable_with_duckdb():
    import duckdb
...
def test_day2_spark_and_lakehouse_poc_artifacts_exist():
    required_jobs = [
    ...
    spark_report = _read_json("reports/day2/spark_bronze_to_silver_market_daily_report.json")
...
def test_backend_license_api_exposes_day2_source_statuses():
    ...
    assert payload["status"] == "day2_license_registry_ready"
...
def test_clickhouse_loader_and_frontend_license_page_are_day2_ready():
    assert (PROJECT_ROOT / "scripts" / "load_day2_clickhouse.py").is_file()
    sql_path = PROJECT_ROOT / "deploy" / "clickhouse" / "day2_ads_tables.sql"
