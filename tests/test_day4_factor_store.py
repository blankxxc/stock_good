...
def _ensure_day4_without_spark() -> tuple[dict, dict]:
...
def test_day4_factor_engine_writes_factor_spec_registry_and_gold_artifacts():
    factor_report, _ = _ensure_day4_without_spark()
...
def test_day4_point_in_time_join_and_risk_model_outputs_are_ready():
    factor_report, pit_report = _ensure_day4_without_spark()
...
def test_day4_spark_materialization_report_matches_polars_outputs():
    _ensure_day4_without_spark()
    report_path = PROJECT_ROOT / "reports" / "day4" / "spark_factor_materialization_report.json"
    if not report_path.exists() or _read_json("reports/day4/spark_factor_materialization_report.json").get("status") != "ok":
        from spark.jobs.day4_factor_materialization import run_job

        spark_report = run_job()
    else:
        spark_report = _read_json("reports/day4/spark_factor_materialization_report.json")
...
def test_day4_backend_apis_and_frontend_factor_page_are_ready():
    _ensure_day4_without_spark()
    ...
    assert factors.json()["status"] == "day4_factor_store_ready"
    assert factors.json()["factor_count"] >= 70
    assert features.json()["status"] == "day4_feature_registry_ready"
    assert features.json()["point_in_time_violations"] == 0
    assert spark_jobs.json()["status"] == "day4_spark_factor_materialization_ready"

    page = (PROJECT_ROOT / "frontend" / "src" / "app" / "factors" / "page.tsx").read_text(encoding="utf-8")
    assert "Day 4" in page
    assert "74 个离线因子" in page
    assert "point_in_time_violations=0" in page
    assert "/api/factors" in page


def test_day4_acceptance_script_reports_ok():
    from scripts.check_day4_acceptance import run_acceptance
