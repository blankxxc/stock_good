...
def test_day3_data_trust_pipeline_generates_quality_lineage_leakage_reports():
    ...
    required_paths = [
        ...
        "reports/day3/day3_data_trust_report.json",
    ...
    assert quality["data_version"] == "day3_v001"
...
def test_day3_quarantine_contains_intentional_synthetic_anomalies_with_required_fields():
    from quality.day3_data_trust import run_day3_data_trust

    run_day3_data_trust()
    quarantine_root = PROJECT_ROOT / "data" / "quarantine" / "day3_synthetic_market"
...
def test_day3_synthetic_mini_market_and_leakage_checker_block_known_traps():
    from quality.day3_data_trust import run_day3_data_trust

    run_day3_data_trust()
    synthetic = _read_json("reports/day3/synthetic_mini_market_report.json")
    ...
    leakage = _read_json("reports/day3/leakage_report.json")
...
def test_day3_lineage_connects_sources_jobs_snapshots_and_quality_reports():
    from quality.day3_data_trust import run_day3_data_trust

    run_day3_data_trust()
    lineage = _read_json("reports/lineage_report.json")
    assert lineage["status"] == "passed"
    assert lineage["data_version"] == "day3_v001"
    ...
    assert any(edge["target_id"] == "reports/day3/leakage_report.json" for edge in edges)


def test_backend_day3_data_quality_and_lineage_apis_are_ready():
    ...
    dq = dq_response.json()
    assert dq["status"] == "day3_data_trust_ready"
    ...
    assert lineage["status"] == "day3_lineage_ready"
    assert lineage["edge_count"] >= 30
    assert lineage["reports"]["json"].endswith("reports/lineage_report.json")


def test_day3_frontend_pages_explain_real_quality_lineage_artifacts():
...
def test_day3_acceptance_script_reports_ok():
    from scripts.check_day3_acceptance import run_acceptance
