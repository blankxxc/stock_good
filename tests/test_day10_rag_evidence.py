...
def _ensure_day10() -> dict:
    from rag.day10_evidence_system import run_day10_rag_pipeline

    return run_day10_rag_pipeline(write_outputs=True)


def test_day10_builds_claim_level_artifacts_and_complete_schema():
    report = _ensure_day10()
...
def test_day10_as_of_present_retrospective_and_license_filters_work():
    _ensure_day10()
    from rag.day10_evidence_system import retrieve_claims
...
def test_day10_answer_constraints_eval_and_backend_frontend_are_ready():
    report = _ensure_day10()
    from backend.app.main import app
    from rag.day10_evidence_system import answer_with_evidence
    from scripts.check_day10_acceptance import run_acceptance
    ...
    payload = rag.json()
    assert payload["status"] == "day10_rag_evidence_ready"
    ...
    assert health.json()["modules"]["rag"] == "day10_claim_evidence_rag_ready"

    page = (PROJECT_ROOT / "frontend" / "src" / "app" / "rag" / "page.tsx").read_text(encoding="utf-8")
    assert "Day 10" in page
