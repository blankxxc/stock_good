...
def test_day12_artifact_builder_creates_paper_simulation_risk_and_export_manifest() -> None:
    from simulation.day12_governance import build_day12_artifacts

    result = build_day12_artifacts(PROJECT_ROOT)
...
def test_day12_rbac_license_and_forbidden_wording_gates_are_enforced() -> None:
...
def test_day12_api_frontend_and_acceptance_script_are_ready() -> None:
    from backend.app.main import app

    client = TestClient(app)
    health = client.get("/health").json()
    assert health["version"] == "0.1.0-day12"
    ...
    acceptance = subprocess.run(
        [sys.executable, "scripts/check_day12_acceptance.py"],
        cwd=PROJECT_ROOT,
        text=True,
        capture_output=True,
        check=True,
