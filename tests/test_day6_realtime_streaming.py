...
def _ensure_day6() -> dict:
    from streaming.flink.day6_realtime_pipeline import run_day6_realtime_pipeline

    return run_day6_realtime_pipeline(write_outputs=True)


def test_day6_topics_producers_and_message_contract_are_ready():
    report = _ensure_day6()
    ...
    topic_health_path = PROJECT_ROOT / "reports" / "day6" / "topic_health.json"
...
def test_day6_flink_like_jobs_emit_realtime_factors_sinks_and_diff_report():
    report = _ensure_day6()
    ...
    factors = pd.read_parquet(PROJECT_ROOT / "reports" / "day6" / "realtime_factor_latest.parquet")
    required_columns = {
        "event_time",
        "trade_date",
        "symbol",
    ...
    assert (PROJECT_ROOT / "reports" / "day6" / "realtime_vs_offline_diff_report.json").is_file()
    assert (PROJECT_ROOT / "reports" / "day6" / "realtime_vs_offline_diff_report.html").is_file()


def test_day6_backend_api_and_frontend_pages_are_ready():
    report = _ensure_day6()
    assert report["status"] == "ok"
    ...
    assert realtime.json()["status"] == "day6_realtime_pipeline_ready"
    ...
    assert flink.json()["status"] == "day6_flink_jobs_ready"
    ...
    assert "Day 6" in realtime_page and "/api/realtime" in realtime_page
    assert "PoC / not formal signal" in realtime_page
    assert "Day 6" in flink_page and "/api/flink-jobs" in flink_page


def test_day6_acceptance_script_reports_ok():
    from scripts.check_day6_acceptance import run_acceptance
