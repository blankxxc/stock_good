...
ALEMBIC_FILES = [
    ...
    "backend/app/db/alembic/versions/0001_foundation_metadata.py",
]


def test_required_foundation_directory_structure_exists():
...
def test_metadata_migration_contains_all_foundation_tables():
    migration = PROJECT_ROOT / "warehouse_schema" / "migrations" / "0001_foundation_metadata.sql"
...
def test_compose_manifest_contains_all_foundation_services():
...
def test_alembic_foundation_migration_scaffold_exists():
    missing_alembic_files = [path for path in ALEMBIC_FILES if not (PROJECT_ROOT / path).is_file()]
    assert missing_alembic_files == []

    migration_text = (PROJECT_ROOT / "backend" / "app" / "db" / "alembic" / "versions" / "0001_foundation_metadata.py").read_text(encoding="utf-8")
...
def test_spark_and_flink_smoke_artifacts_exist():
    assert (PROJECT_ROOT / "spark" / "jobs" / "foundation_spark_smoke.py").is_file()
    assert (PROJECT_ROOT / "spark" / "conf" / "spark-defaults.conf").is_file()
    assert (PROJECT_ROOT / "streaming" / "flink" / "foundation_flink_job_graph.py").is_file()
