...
def _ensure_advanced_models() -> dict:
    from models.advanced_models_advanced_models import run_advanced_models_advanced_model_pipeline

    return run_advanced_models_advanced_model_pipeline(write_outputs=True)


def test_advanced_models_all_advanced_model_adapters_have_small_sample_runs_and_candidate_gate():
    report = _ensure_advanced_models()
...
def test_advanced_models_predictions_comparison_and_experiment_artifacts_are_written():
    report = _ensure_advanced_models()
...
def test_advanced_models_backend_frontend_and_acceptance_are_ready():
    report = _ensure_advanced_models()
    ...
    payload = models.json()
    assert payload["status"] == "advanced_models_advanced_models_ready"
    assert set(payload["models"].keys()) == REQUIRED_MODELS
    assert payload["approval_status"] == "research_candidate_only_not_approved"
    assert experiments.status_code == 200
    assert experiments.json()["advanced_models"]["status"] == "advanced_models_advanced_models_ready"
    assert health.status_code == 200
    assert health.json()["modules"]["advanced_models"] == "advanced_models_research_candidate_adapters_ready"

    page = (PROJECT_ROOT / "frontend" / "src" / "app" / "models" / "page.tsx").read_text(encoding="utf-8")
    assert "advanced_models" in page
    ...
    from scripts.check_advanced_models_acceptance import run_acceptance
