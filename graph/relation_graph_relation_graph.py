...
DATA_VERSION = "relation_graph_synthetic_relation_graph_v001"
SCHEMA_VERSION = "v0.8.0"
RELATION_EDGE_VERSION = "stock_relation_edge_relation_graph_v001"
RELATION_FACTOR_VERSION = "relation_factor_relation_graph_v001"
FEATURE_SET_VERSION = "feature_set_relation_graph_relation_graph_v001"
RUN_ID = "relation_graph_relation_graph_v001"

relation_graph_DIR = ROOT / "reports" / "relation_graph"
EDGE_DIR = ROOT / "data" / "gold" / "stock_relation_edge"
RELATION_FACTOR_DIR = ROOT / "data" / "gold" / "factor_relation_panel"
RELATION_DAILY_DIR = ROOT / "data" / "gold" / "factor_daily_panel_relation_graph_relation"
FEATURE_relation_graph_DIR = ROOT / "data" / "gold" / "model_feature_matrix_wide_relation_graph"
...
def _ensure_event_regime_feature() -> pd.DataFrame:
    from models.event_regime_event_regime_ablation import run_event_regime_event_regime_pipeline

    feature_dir = ROOT / "data" / "gold" / "model_feature_matrix_wide_event_regime"
    if not list(feature_dir.glob("**/*.parquet")):
        run_event_regime_event_regime_pipeline(write_outputs=True)
...
def build_stock_relation_edges(feature: pd.DataFrame, write_outputs: bool = True) -> pd.DataFrame:
    ...
    # event_regime event documents are mostly single-stock; synthesize co-mention edges inside the same affected industry/concept bucket.
    for idx, src in enumerate(symbols):
        for dst in symbols[idx + 1 : idx + 4]:
            if industry_map.get(src) == industry_map.get(dst) or concept_map.get(src) == concept_map.get(dst):
                rows.append(_edge_row(latest_date, src, dst, "news_co_mention", 0.58, 0.72, "synthetic_event_regime_event_bucket"))
                rows.append(_edge_row(latest_date, dst, src, "news_co_mention", 0.58, 0.72, "synthetic_event_regime_event_bucket"))
    ...
    if write_outputs:
        ...
        _write_json(relation_graph_DIR / "spark_price_corr_edge_job.json", {
...
def build_relation_factor_panel(feature: pd.DataFrame, edges: pd.DataFrame, write_outputs: bool = True) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if write_outputs:
        ...
        _write_json(relation_graph_DIR / "graph_summary.json", {"status": "ok", **graph_summary, "networkx_status": "networkx_centrality_ready"})
    return relation, daily


def build_relation_graph_feature_matrix(feature: pd.DataFrame, relation: pd.DataFrame, write_outputs: bool = True) -> pd.DataFrame:
    relation_cols = [
        "neighbor_return_5m",
        "neighbor_return_1d",
        "neighbor_volume_shock",
    ...
    if write_outputs:
        _write_parquet_dir(merged, FEATURE_relation_graph_DIR)
    return merged
...
def build_relation_ablation(feature: pd.DataFrame, write_outputs: bool = True) -> dict[str, Any]:
    ...
    configs = {
        "base_event_regime": score(base_cols),
        "base_plus_relation_graph": score(base_cols + relation_cols),
        "full_minus_relation_graph": score(base_cols),
    }
    gain = configs["base_plus_relation_graph"]["rank_ic_smoke"] - configs["base_event_regime"]["rank_ic_smoke"]
    report = {
        "status": "ok",
        "run_id": "relation_graph_relation_factor_ablation_v001",
    ...
    if write_outputs:
        _write_json(relation_graph_DIR / "relation_factor_ablation_report.json", report)
    return report


def run_relation_graph_relation_graph_pipeline(write_outputs: bool = True) -> dict[str, Any]:
    ...
    feature8 = build_relation_graph_feature_matrix(feature, relation, write_outputs=write_outputs)
    ...
    report = {
        ...
        "artifacts": {
            ...
            "factor_daily_panel_relation_graph_relation": "data/gold/factor_daily_panel_relation_graph_relation",
            "model_feature_matrix_wide_relation_graph": "data/gold/model_feature_matrix_wide_relation_graph",
            "hist_trsr_adapter": "data/gold/graph_model_adapters/hist_trsr",
            "relation_ablation_report": "reports/relation_graph/relation_factor_ablation_report.json",
            "graph_summary": "reports/relation_graph/graph_summary.json",
            "spark_price_corr_job": "reports/relation_graph/spark_price_corr_edge_job.json",
        },
    }
    if write_outputs:
        _write_json(relation_graph_DIR / "relation_graph_relation_graph_report.json", report)
        html = f"""<!doctype html><html><head><meta charset='utf-8'><title>relation_graph Relation Graph</title></head><body><h1>relation_graph Relation Graph</h1><p>Status: {report['status']}</p><p>Edges: {report['edge_rows']}</p><p>Relation types: {', '.join(relation_types)}</p><p>Boundary: {RESEARCH_BOUNDARY}</p></body></html>"""
        (relation_graph_DIR / "relation_graph_report.html").write_text(html, encoding="utf-8")
    return report


if __name__ == "__main__":
    print(json.dumps(run_relation_graph_relation_graph_pipeline(write_outputs=True), ensure_ascii=False, indent=2, default=_json_default))
